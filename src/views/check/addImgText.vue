<template>
  <div class="app-container">
    ...
    <el-form :inline="true" :model="formData" label-width="120px">
      ...
      <el-form-item class="width-25" label="发布标签">
        <el-select v-model="formData.labelId" placeholder="请选择标签" class="inp-w">
          <el-option
            v-for="item in releasetoolDictList"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          />
...
<script>
import { getVirtualSelect, addImgText } from '../../api/check'
import { toolDictList } from '../../api/check'
export default {
  data() {
    return {
  ...
  methods: {
    uploadCoverImg(res) {
      this.$set(this.formData, 'coverUrlList', [
        ...this.formData.coverUrlList,
        res.data
      ])
    },
    removeCoverImg(file, list) {
      const coverUrlList = JSON.parse(
        JSON.stringify(this.formData.coverUrlList)
      )
    ...
    handleAdd() {
      ...
      addImgText(formData)
        ...
        .then(_ => {
          ...
          this.formData = { type: '1', coverUrlList: [] }
          this.$refs.upload
    ...
    init() {
      this.gettoolDictList()
    },
    gettoolDictList() {
      toolDictList().then(res => {
        res.data.splice(0, 1)
        this.releasetoolDictList = res.data
      })
    }
  }
}
