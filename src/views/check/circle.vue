  ...
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-select v-model="listQuery.type" placeholder="类型" class="filter-item">
        <el-option
          v-for="item in [{value: 0, label: '全部'},{value: 1, label: '图片'}, {value: 2, label: '视频'}]"
          :key="item.value"
          :label="item.label"
          :value="item.value"
        />
      ...
      <!-- 搜索功能 end -->
      <el-button class="filter-item" type="primary" @click="handleAdd">新增圈子动态</el-button>
    </div>
    <el-table
    ...
    >
      ...
      <el-table-column label="发布内容" align="center" width="300">
        <template slot-scope="scope">{{ scope.row.content }}</template>
      </el-table-column>
      <el-table-column label="图片视频" align="center" width="300">
    ...
    <!-- 分页功能 end -->
    <el-dialog :visible.sync="showPreImg" title="图片" width="88%">
...
<script>
import { getCircle, deleteCircle } from '../../api/check'
...
export default {
  ...
  methods: {
    getList() {
      ...
      getCircle(this.listQuery).then(response => {
    ...
    handleFilter() {
      ...
      this.getList()
      this.$store.commit('setSearchlistQz', this.listQuery)
    },
    setScreenList() {
      if (validate.validatANo(this.$store.state.search.searchlistQz)) {
        this.listQuery = this.$store.state.search.searchlistQz
      }
    }
  },
  ...
  handleCurrentChange(val) {
    ...
    this.getList()
    this.$store.commit('searchlistQz', this.listQuery)
  },
  // 搜索和分页 end
  handleGoDetail(row) {
  ...
  handleAdd() {
    this.$router.push('/check/addCircleDynamic')
  }
}
</script>
