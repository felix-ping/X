<template>
  <div class="app-container">
    <div class="filter-container">
      <!-- 搜索功能 begin -->
      <el-input v-model="listQuery.summary" placeholder="活动名称" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter"/>
    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="Loading" border fit highlight-current-row @sort-change="handleSort">
      ...
      <el-table-column align="center" label="活动名称" width="150" prop="age">
        <template slot-scope="scope">
          {{ scope.row.summary }}
        </template>
      </el-table-column>
      <el-table-column label="活动简介" align="center" prop="mobile">
        <template slot-scope="scope">
          {{ scope.row.remark }}
        </template>
      </el-table-column>
...
<script>
import { getStoreActivity, deleteStoreActivity, addOrEditStoreActivity } from '../../api/activity'
...
export default {
  ...
  methods: {
    getList() {
      ...
      getStoreActivity(this.listQuery).then(response => {
    ...
    // 搜索和分页 end
    handleDialogForAdd() {
      this.$router.push('/activity/storeActivityDetail')
    },
    // 修改页面
    handleEdit(row) {
      this.$router.push('/activity/storeActivityDetail?id=' + row.id)
    },
    // 删除此条数据
    handleDelete(row) {
      deleteStoreActivity(row.id).then(res => {
        successMsg('删除成功')
        this.getList()
      })
    },
    ...
    handleSetEnable(row) {
      ...
      addOrEditStoreActivity(data).then(() => {
        successMsg('设置状态成功')
        this.getList()
      })
    },
